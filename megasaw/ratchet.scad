...
// inner (teeth) ring
inner_ring_diameter=60;		// outer diameter of the inner ring
...
module outer(
  ...
  difference() {
          ...
          for (n = [0:arms]) {
            nth_axle_transform(n) {
              ...
              rotate(min_rotation() - atan2(axle_r + socket_thickness - (arm_w / 2), arm_l + axle_r) - 90) {
                translate([-arm_l, -axle_d/2-socket_thickness, 0]) {
                  cube([arm_l, spring_thickness, outer_ring_height]);
                }
              }
            }
          }
    ...
    for (n = [0:arms]) {
      nth_axle_transform(n) {
        translate([0, 0, -1]) {
          cylinder(d = axle_d + (extrusion_width * 2), h=outer_ring_height + 2);
        }
      }
    }
  }
...
union() {
  difference() {
    ...
    inner(inner_ring_height = inner_ring_height - 2 * washer_thickness);
    difference() {
    ...
    // Center bore
    translate([0,0,-1]) {
      cylinder(h=inner_ring_height + 2, d = 9 + extrusion_width, $fn = 48);
    }
  }
  /*
  outer();
